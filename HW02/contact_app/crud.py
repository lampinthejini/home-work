...
def refresh_login_session(db: Session, session: models.LoginSession) -> models.LoginSession:
...
def create_category(db: Session, user_id: int, name: str) -> models.Category:
    ...
    db.add(category)
    _commit_or_conflict(db, "이미 존재하는 카테고리 이름입니다.")
    db.refresh(category)
    return category
...
def delete_category(db: Session, category: models.Category) -> None:
    ...
    if len(list_categories(db, category.user_id)) <= 1:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="최소 하나의 카테고리는 남아있어야 합니다.",
        )
    db.delete(category)
    db.commit()
...
def list_contacts(
    ...
    stmt = (
        ...
        .options(selectinload(models.Contact.category))
    )
    if name:
...
def create_contact(db: Session, user_id: int, data: schemas.ContactCreate) -> models.Contact:
    ...
    db.add(contact)
    _commit_or_conflict(db, "이미 등록된 전화번호입니다.")
    db.refresh(contact)
    return contact
